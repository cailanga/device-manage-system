package cn.pzhxy.devicemanager.record.service.impl;
...
import cn.pzhxy.devicemanager.record.mapper.UseRecordMapper;
...
public class UseRecordServiceImpl extends BaseServiceImpl<UseRecord> implements IUseRecordService {
    ...
    @Override
    public PageList<UseRecord> pageList(BaseQuery query) {
        List<UseRecord> useRecords = useRecordMapper.queryPageList(query);
        Long total = useRecordMapper.queryTotal(query);
        for (UseRecord useRecord : useRecords) {
            Employee operator = useRecord.getOperator();
            operator = employeeMapper.selectById(useRecord.getOperatorId());
            useRecord.setOperator(operator);
